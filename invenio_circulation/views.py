...
from .errors import InvalidLoanState, ItemNotAvailable, PropertyRequired
...
class ItemLoanResource(ContentNegotiatedMethodView):
    ...
    def get(self, *args, **kwargs):
        ...
        if not item_pid:
            raise ItemNotAvailable(item_pid=item_pid)
...
def validate_replace_item(loan, data):
    ...
    if loan["state"] not in active_states:
        raise InvalidLoanState(description=(
    ...
    if not item_pid:
        raise PropertyRequired(description="Property item_pid is required.")
